...
export const securityMiddleware = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const validRoles: RateLimitRole[] = ['admin', 'teacher', 'student', 'guest'];
        const userRole = req.user?.role;
        const role: RateLimitRole = validRoles.includes(userRole as RateLimitRole) ? userRole as RateLimitRole : 'guest';
